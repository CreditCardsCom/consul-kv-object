...
function consulKvObjectForge(kvSync, userOptions) {
    ...
    function consulResponseToObject(err, res, prefix, callback) {
        ...
        if (err) {
            ...
            const response = res.reduce(function reducePathToObject(assembly, consulKey) {
                let keyPath = consulKey.Key.substr(prefixLen).split('/');
    ...
    function set(keyOptions, values, callback) {
        ...
        const promisedSet = keyList.map(function (o) {
            let options = wrapCosnsulOptions({}, o.key, o.value, o.flag);
            return kv.setAsync(options);
        });

        if (callback) {
            return Bluebird.all(promisedSet).then((data) => callback(null, data));
        }
    }
