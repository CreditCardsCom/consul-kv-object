...
function consulKvObjectForge(kvSync, userOptions) {
    ...
    const kv = Bluebird.promisifyAll(kvSync);

    const consulObjectOptions = Object.keys(defaultOptions)
        .reduce(
    ...
    function objectToKeyList(object, prefix, target) {
        target = target || [];
        var key;
        if (object !== null && object.constructor === Object) {
            for( key in object )
            {
                 if ( object.hasOwnProperty(key) ) {
                     objectToKeyList(object[key], (prefix ? prefix + "/" : "") + key, target );
                 }
            }

        } else {
            target.push(createKey(object, prefix));
        }
        return target;
    }
    ...
    function set(keyOptions, values, callback) {
        ...
        const promisedSet =
            Bluebird.map( keyList, 
                function (o) {
                    const options = wrapCosnsulOptions({}, o.key, o.value, o.flag);
                    const promiseOfSet = kv.setAsync(options);
                    if (consulObjectOptions.delay > 0) {
                        return promiseOfSet.delay( consulObjectOptions.delay )
                    }
                    else {
                        return promiseOfSet;
                    }
                ...
                {
                    concurrency: consulObjectOptions.concurrency
                });

        if (callback) {
            return promisedSet.then((data) => callback(null, data));
        }
    }
