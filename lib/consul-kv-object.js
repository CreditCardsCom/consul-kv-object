...
function consulKvObjectForge(kv, userOptions) {
    ...
    var consulObjectOptions = Object.keys(userOptions || {})
        .reduce(
    ...
    function createValue(consulKey) {
        if (consulObjectOptions.typeMapFlags && consulObjectOptions.mapTypes) {
            var type = consulObjectOptions.typeMapFlags[consulKey.Flags] || consulObjectOptions.defaultType;
    ...
    function get(options, callback) {
        const consulRequestOptions = wrapCosnsulOptions(options);
        return kv.get(consulRequestOptions, function (err, res) {
            assembleConsulResponse(err, res, consulRequestOptions.key, callback)
        });
    }
